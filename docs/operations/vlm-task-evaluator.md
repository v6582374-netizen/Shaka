...
单次调用运行器会复制并摘要绑定 `evaluator.json` 与相邻的冻结
`prompt.md`，在证据完成与身份校验之后调用本页的准备和评估逻辑。模型
原始结果作为独立工件保存；后续人工审校只引用其摘要，不覆盖模型事实
或五态结果。
...
程序按照统一时间戳选取回合窗口，将每个时间点的头部左右视角和左右腕视角组成一张四视角面板，并保存面板来源、时间误差和 SHA-256。准备清单同时绑定原始调用的 `sha256.txt`、采集元数据摘要、控制器结果、控制时间窗和完整性结论；它不重新解释完整性规则。原始调用目录按相对于准备证据目录的路径记录，因此运行目录原子发布后仍可重新评估；评估器也继续兼容已有的绝对路径清单。
