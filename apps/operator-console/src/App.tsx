...
import { G1MonitorView } from "./components/G1MonitorView";
...
export function App() {
  ...
  if (prototype === "g1-status") return <G1StatusSidebarPrototype />;
...
function VegapunkApp() {
  ...
  // A browser-only session has no server to resume from. Reuse its local identity across a
  // reload so UI preferences tied to the active session remain stable without a health check.
  const [sessionId, setSessionId] = useState<string>(
    () => readLastSessions().cowork?.sessionId ?? newId(),
  );
  ...
  // The browser shell is usable without the retired sidecar. Keep service reads opportunistic,
  // but never make them a prerequisite for rendering or choosing a local workspace.
  const [onboarding, setOnboarding] = useState(false);

  const loadSettings = () =>
    getSettings()
      .then((s) => {
        // Settings, when available, are the single source of truth for the selected model.
        // This replaces the retired health-probe dependency; the local default remains usable
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sessionId, agent, refreshSessions]);
