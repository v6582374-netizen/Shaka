...
describe("Physical AI navigation group", () => {
  it("groups Body Status, the governance workbench, and Camera under one Physical AI parent", async () => {
    stubFetch([
      { match: "/v1/personas", method: "GET", json: PERSONAS },
      { match: "/v1/settings", method: "GET", json: { nav_layout: "flat" } },
    ]);
    ...
    expect(within(items).getByRole("button", { name: "Body Status" })).toBeTruthy();
    ...
    expect(screen.queryByRole("button", { name: "Body Status" })).toBeNull();
  ...
  it("opens each Physical AI surface from its own entry", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: "Body Status" }));
    expect(baseProps.onOpenG1Monitor).toHaveBeenCalledOnce();

    fireEvent.click(screen.getByRole("button", { name: "Embodied" }));
